...
from chia_puzzles.manage_clvm import (Cache, ClvmBytes, ClvmPaths,
                                      ManageClvmError, create_cache_entry,
                                      create_empty_cache, dump_cache,
                                      find_stems, load_cache)
...
class WrongCacheVersionError(CacheVersionError):
    def __init__(self, found_version: object, expected_version: CacheVersion) -> None:
        ...
        super().__init__(
            f"Cache has wrong version, expected {expected_version!r} got: {found_version!r}"
        )
...
def test_check(use_cache: bool) -> int:
    ...
    HASHES: typing.Dict[str, str] = (
        json.loads(hashes_path.read_text()) if hashes_path.exists() else {}
    )
    ...
    if not use_cache:
        ...
        try:
            ...
            print(
                f"Ignoring cache due to incorrect version, expected {e.expected_version!r} got: {e.found_version!r}"
            )
            cache = create_empty_cache()
        ...
        with open(stem_path.with_name(stem_path.name + clvm_suffix)) as file:
            ...
            for line in file_lines:
                ...
                if "(" in non_comment:
                    ...
                    if (
                        len(non_comment) >= paren_index + 4
                        and non_comment[paren_index : paren_index + 4] == "(mod"
                    ):
                        overall_fail = True
                        print(
                            f"FAIL    : {stem_path.name + clvm_suffix} contains `(mod`"
                        )
                    break
    ...
    print()
    print(
        "Checking that all existing .clsp files compile to .clsp.hex that match existing caches:"
    )
            ...
            if not cache_hit:
                with tempfile.TemporaryDirectory() as temporary_directory:
                    generated_paths = ClvmPaths.from_clvm(
                        clvm=pathlib.Path(temporary_directory).joinpath(
                            reference_paths.clvm.name
                        ),
                        hash_dict=HASHES,
                    )
                    compile_clvm(
                        ...
                        search_paths=[
                            os.fspath(reference_paths.clvm.parent),
                            str(std_libraries),
                        ],
                    )

                    generated_bytes = ClvmBytes.from_hex_bytes(
                        hex_bytes=generated_paths.hex.read_bytes()
                    )
